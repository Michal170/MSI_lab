...
clf = RandomSubspaceEnsemble(base_estimator=GaussianNB(), n_estimator=10, n_subspace_features=5,hard_voting=True, random_state=123)
scores = []
...
clf = RandomSubspaceEnsemble(base_estimator=GaussianNB(), n_estimator=10, n_subspace_features=5,hard_voting=False, random_state=123)
scores = []
for train, test in rskf.split(X,y):
    clf.fit(X[train], y[train])
    y_pred = clf.predict(X[test])
